...
import {Protocol, open, type SimConnectConnection} from 'node-simconnect';
import { getDistanceBetweenPoints as dist } from "./utils";
...
import {type MsfsApi} from '../msfs.api';
...
const degrees = (rad: number) => (rad / PI) * 180;
const radians = (deg: number) => (deg / 180) * PI;

const NEARBY_AIRPORTS = `NEARBY AIRPORTS`;
const ALL_AIRPORTS = `ALL AIRPORTS`;
const AIRPORT = `AIRPORT:`;
const SPECIAL_VARS = [ALL_AIRPORTS, NEARBY_AIRPORTS, AIRPORT] as const;
 ...
 * ...docs go here...
 */
export async function getAirportHandler(api: MsfsApi, handle: SimConnectConnection) {
